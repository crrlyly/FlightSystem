...
try {
    ...
    String numStopsStr = request.getParameter("numStops");
    ...
    String dayBitsStr = request.getParameter("dayBits");

    StringBuilder query = new StringBuilder("SELECT * FROM flight WHERE 1=1");
    List<Object> params = new ArrayList<>();
    
    if (!flightNumsList.isEmpty()) {
        query.append(" AND flightNum IN (");
        for (int i = 0; i < flightNumsList.size(); i++) {
            if (i > 0) query.append(",");
            query.append("?");
        }
        query.append(")");
        ...
        for (String f : flightNumsList) {
            params.add(Integer.parseInt(f));
        }
    }
    ...
    /*if (numStopsStr != null && !numStopsStr.isEmpty()) {
        if (numStopsStr.equals("2")) {
            query.append(" AND numStops >= 2");
        } else {
            query.append(" AND numStops = ?");
            params.add(Integer.parseInt(numStopsStr));
        }
    }*/
    ...
    if (sortBy != null && sortOrder != null &&
   	    !sortBy.equals("none") && !sortOrder.equals("none")) {

   	    if (sortBy.equals("duration")) {
   	        query.append(" ORDER BY TIME_TO_SEC(arrival_time) - TIME_TO_SEC(departure_time) ")
